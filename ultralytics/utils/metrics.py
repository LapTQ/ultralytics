...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    @property
    def results_dict(self, **kwargs) -> dict[str, float]:
        """Returns dictionary of computed performance metrics and statistics."""
        keys = [*self.keys, "fitness"] + ['metrics__details__by__id_class']
        values = ((float(x) if hasattr(x, "item") else x) for x in ([*self.mean_results(), self.fitness] + [self.get__metrics__details__by__id_class()]))
